        ...
        <div class="team-wrap">
            ...
            <div class="team-hero">
                ...
                <div class="hero-content">
                    ...
                    <div class="hero-info">
                        <p class="hero-label">{{ team.type === 'national' ? t('ui.national_team') : t('ui.club') }}</p>
                        <h1 class="hero-title">{{ teamName(team) }}</h1>
                        <p class="hero-sub">{{ matches.total }} {{ t('ui.matches_available') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="section-head">
                <div class="section-head-left">
                    ...
                    <div>
                        <h2 class="section-title">{{ t('ui.match_highlights') }}</h2>
            ...
            <div v-if="matchList.length === 0" class="empty">
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#444" stroke-width="1.5" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>
                <p>{{ t('ui.no_matches_team') }}</p>
            </div>
                ...
                >
                    ...
                    <div class="card-info">
                        ...
                        <div class="card-teams-row">
                            <span class="ctr-name" :class="{ 'ctr-highlighted': match.home_team?.id === team.id }">{{ teamName(match.home_team) }}</span>
...
const { teamName, localePath, t, formatDate, formatRound, formatPageLabel } = useLocale(toRef(props, 'locale'))
const matchList = computed(() => props.matches?.data ?? [])
const { teamSeo } = useSeo(toRef(props, 'locale'))
const seo = computed(() => teamSeo(props.team))
...
.team-hero {
    ...
    overflow: hidden;
    background: linear-gradient(135deg, #1a1a2e 0%, #0b0b10 60%, #16213e 100%);
    margin-bottom: 24px;
}
@media (max-width: 768px) { .team-hero { height: 140px; } }

.hero-overlay {
    position: absolute; inset: 0;
    background: linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, rgba(11,11,16,0.9) 100%);
    z-index: 0;
}
.hero-content {
...
.hero-logo span { font-size: 20px; font-weight: 700; color: #1a1a1a; }
...
/* ── Card info ── */
.card-info { padding: 8px 10px 10px; display: flex; flex-direction: column; gap: 5px; }

.card-league { display: flex; align-items: center; gap: 5px; }
.lg-icon { width: 14px; height: 14px; object-fit: contain; }
.lg-name-text { font-size: 10px; font-weight: 600; color: #ef4444; text-transform: uppercase; letter-spacing: 0.04em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

.card-teams-row { display: flex; align-items: center; gap: 5px; flex-wrap: wrap; }
.ctr-name { font-size: 11.5px; font-weight: 700; color: #d0d0dc; text-align: center; }
.ctr-name.ctr-highlighted { color: #fff; }
